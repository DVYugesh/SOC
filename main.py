...
TUPLES = [
    ...
    (5, 6, 9, 10), (6, 7, 10, 11), (8, 9, 12, 13), (9, 10, 13, 14),
    (10, 11, 14, 15)
]
    ...
    try:
        for i_se in range(n_session):
            for i_ep in range(n_episode):
                gp = play(agent, board=None, spawn_random_tile=True)
                n_games += 1
                print(f"Session {i_se}, Episode {i_ep}, Reward: {gp.game_reward}, Max Tile: {gp.max_tile}")
        ...
        if input("save the agent? (y/n)") == "y":
            ...
            with open(fout, "wb") as file:
                pickle.dump((n_games, agent), file)
